...
const ProjectsCard = ({ id, index, title, description, photo }) => {
  ...
  return (
    ...
    >
      <div className={`bg-[#272A57] w-full h-full overflow-hidden ${id === 'Project-1' || id === 'Project-7' ? 'h-44 flex-1' : 'flex-[0.8]'}`}>
        <img src={photo} className='w-full h-full object-cover' alt={title} loading='lazy' />
      </div>
      <div className='flex-1 p-8'>
        <h1 className='text-3xl text-[#272A57] font-bold'>{title}</h1>
        <p className='text-sm text-[#272A57] mt-3'>
          {showFullDescription ? description : truncatedDescription}
        </p>
        {!isBigCard ?
          <p
            className='flex w-full justify-end items-center gap-1 text-xs text-[#272A57] opacity-80 mt-auto absolute cursor-pointer bottom-4 right-4'
